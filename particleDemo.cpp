...
const float a = 17.834f;
const float e = 0.96714f;
const float b = a*(1.f - e*e);
const float g = 0.000001f;
const float m = 1.f;
const float n = 2.f*3.1416f*sqrt(g*m/powf(a,3));
float M = 0.f;
float E = 0.f;
...
const char *sSDKsample = "CUDA Particles";
...
//uint numParticlesFunc(float time){
//	uint n = expf(-(time-numParticles)/2.f);
//	return n;
//}
...
void display()
{
    ...
    //uint numParticles = numParticlesFunc(-(currentTime - initnumParticles));
    if (!paused)
    {
        if (emitterOn)
        {
        	if (currentTime <= 10000.f)
        	{
        		runEmitterInit();
        		//runCurrentDensity();
    ...
    if (fpsCount >= fpsLimit)
    {
        ...
        sprintf(fps, "CUDA Particles (%d particles): %3.1f fps, time = %f", numParticles, ifps, currentTime);
...
void runEmitterInit()
{
    ...
    psystem->sphereEmitterInit(emitterIndex, cursorPosLag, emitterRadius, ftoi(emitterRate*timestep), particleLifetime);
...
void runCurrentDensity()
{
    ...
    //vec3f vel = vec3f(0, 0, 0);
	psystem->currentDensity(emitterIndex, make_uint3(s, s, s), ftoi(emitterRate*timestep), cursorPosLag);
...
//        E = M/powf(1.f-e,1) -
//        		(e/powf(1.f-e,4))*(powf(M,3)/fact(3)) +
//        		(((9.f*e*e)+e)/powf(1.f-e,7))*(powf(M,5)/fact(5));// +
//        		(((225.f*powf(e,3))+(54.f*powf(e,2))+e)/powf(1.f-e,10))*(powf(M,7)/fact(7));// +
//        		(((11025.f*e*e*e*e)+(4131.f*e*e*e)+(243.f*e*e)+e)/powf(1.f-e,13))*(powf(M,9)/fact(9));

 //       cursorPos.x = b*sin(E);
//        cursorPos.y = 0.0f;
//        cursorPos.z = a*(cos(E) - e);
