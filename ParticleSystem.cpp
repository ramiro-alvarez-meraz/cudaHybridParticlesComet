...
ParticleSystem::_initialize(int numParticles, bool bUseGL)
{
    ...
    createCurrentTexture(64, 64, 64);
    createMagneticFieldTexture(64, 64, 64);
    createElectricFieldTexture(64, 64, 64);
...
// current
//void
//ParticleSystem::chargeDensity(vec3f vel)
//{
//
//}

/*
// derivative of an arbitrary field
void
particleSystem::deriv(vec3f start, uint size, uint numParticles)
{
    for (uint z=0; z<size.z; z++)
    {
        for (uint y=0; y<size.y; y++)
        ...
        case CONFIG_GRID:
            {
                ...
                initGrid(vec3f(-1.0, 0.0, -1.0), make_uint3(s, s, s), vec3f(m_particleRadius*2.0f), jitter, vec3f(0.0), m_numParticles, 1000.0);
            }
            break;
    }
...
ParticleSystem::discEmitter(uint &index, vec3f pos, vec3f vel, vec3f vx, vec3f vy, float r, int n, float lifetime, float lifetimeVariance)
{
    ...
    for (int i=0; i<n; i++)
    {
        ...
        if (index < m_numParticles)
        {
            ...
            posPtr[index] = make_float4(p.x, p.y, p.z, 0.f);
...
void
ParticleSystem::sphereEmitterInit(uint &index, vec3f pos, float r, int n, float lifetime)
{
    float4 *posPtr = m_pos.getHostPtr();
    float4 *velPtr = m_vel.getHostPtr();
    ...
    {
        vec3f x = randSphere();
        float distN = length(x)/r;

        if (index < m_numParticles)
        {
            //vec3f p = pos + x/(expf(-distN)/(distN*distN));
            vec3f p = pos + x*(distN);
            float dist = length(p);
            float age = 0.f;
            float lt = lifetime*1000.f;
            vec3f v = {0.1f*(p.x/powf(dist,1)), 0.1f*(p.y/powf(dist,1)), 0.1f*(p.z/powf(dist,1))};
...
void
ParticleSystem::currentDensity(uint &index, uint3 size, int n, vec3f pos)
{
    //float4 *posPtr = m_pos.getHostPtr();
    float4 *velPtr = m_vel.getHostPtr();
	float3 *densPtr = m_dens.getHostPtr();
	...
	for (uint z=0; z<size.z; z++)
	{
	    for (uint y=0; y<size.y; y++)
		{
		    for (uint x=0; x<size.x; x++)
		    {
		        uint l = (z*size.y*size.x) + (y*size.x) + x;
		        ...
		        if (index < m_numParticles)
		        {
		        	...
		        	densPtr[l].x += velPtr[index].x;
		        	densPtr[l].y += velPtr[index].y;
		        	densPtr[l].z += velPtr[index].z;
		        	index++;
			        //if (l==0)printf("%.20f\t%d\t%d\n",densPtr[l].y, countCell, index);
		        }
   ...
   // m_dens.copy(GpuArray<float3>::HOST_TO_DEVICE, startCell, countCell);
}
/*
void
ParticleSystem::sphereEmitter(uint &index, vec3f pos, vec3f vel, vec3f spread, float r, int n, float lifetime, float lifetimeVariance)
{
    ...
    for (int i=0; i<n; i++)
    {
        ...
        if (index < m_numParticles)
        {
            ...
            vec3f p = pos + x/(5.f*expf(-distN)/(distN*distN));
            float age = 0.f;
            ...
            //vec3f v = vel;
            //vec3f v = (vel + dir*spread)/length(pos);
            vec3f v = vel;
